...
const BLUE = "#4d9fff"; const BLUE_T = "#70b8ff"; const BLUE_G = "rgba(77,159,255,0.35)";
const ORANGE = "#FF3E00";
...
const eyebrow: React.CSSProperties = { ...MONO, fontSize: 12, fontWeight: 500, letterSpacing: "0.18em", textTransform: "uppercase", color: BLUE_T, marginBottom: 20 };
  ...
  return (
    <>
      ...
      <section className="mkt-hero mkt-hero--pillar">
        <HubSpokeBg centerX={0.75} centerY={0.55} scale={0.38} overlay={false} color={{ r: 77, g: 159, b: 255 }} />
        <div className="mkt-hero-eyebrow" style={{ color: BLUE_T, background: "rgba(77,159,255,0.1)", borderColor: "rgba(77,159,255,0.22)" } as React.CSSProperties}>
          For Operators and Organizations
        </div>
        <h1 className="mkt-hero-headline">
          Unified scheduling<br /><span style={{ color: BLUE, textShadow: `0 0 50px ${BLUE_G}` }}>intelligence.</span>
        </h1>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 2, maxWidth: 600, marginTop: 8 }}>
          {[
            ...
            <div key={p.n} style={{ background: BG2, border: `1px solid ${BORDER}`, padding: "16px 18px" }}>
              <div style={{ ...MONO, fontSize: 10, letterSpacing: "0.14em", textTransform: "uppercase", color: BLUE_T, marginBottom: 6 }}>{p.n}</div>
              <div style={{ fontSize: 15, fontWeight: 700, color: "#fff" }}>{p.t}</div>
            </div>
          ))}
        </div>
        ...
        <div style={inner}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
            {[
              ...
              { badge: "Youth sports",             t: "Large club and league organizations",   d: "Multiple scheduling platforms across leagues, tournaments, and communications — manually reconciled every week by staff who should be doing something more valuable." },
              { badge: "Recreation",               t: "Municipal parks and recreation",        d: "City-managed facilities serving multiple sports, leagues, and programs across multiple scheduling systems, with staff hours going into data entry instead of community programs." },
        ...
        <div style={inner}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 56, alignItems: "start" }}>
            <div>
              ...
              <p style={body}>The problem is not your tools. <span style={{ color: ORANGE, fontWeight: 600 }}>The problem is that they were never designed to talk to each other.</span> Orgo Sync is the layer that makes them.</p>
            </div>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              {[
                ...
                <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 14, padding: "18px 20px", borderRadius: 12, background: "rgba(255,255,255,0.025)", border: "1px solid rgba(255,255,255,0.07)" }}>
                  <div style={{ width: 20, height: 20, borderRadius: "50%", background: "rgba(255,77,77,0.12)", border: "1px solid rgba(255,77,77,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, marginTop: 2, fontSize: 10, fontWeight: 700, color: "#ff6b6b" }}>✕</div>
                  <div style={{ fontSize: 16, color: W90, lineHeight: 1.65 }}>{item}</div>
                </div>
              ))}
            </div>
          </div>
        ...
        <div style={inner}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: 20 }}>
            {[
              ...
              { t: "Add systems without adding complexity", d: "Every new platform you add connects to Orgo Sync once and joins the unified view automatically. Your stack can grow without the reconciliation cost growing with it." },
              { t: "One view across everything",            d: "Daily, weekly, and monthly views across every system your organization runs. Desktop and fully responsive on mobile. The single source of truth your organization has never had." },
            ...
            {[
              ...
              <div key={c.tier} style={{ borderRadius: 16, padding: 28, display: "flex", flexDirection: "column", gap: 12, background: c.featured ? "rgba(77,159,255,0.06)" : BG2, border: c.featured ? "1.5px solid rgba(77,159,255,0.35)" : `1px solid ${BORDER}` }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
                  {c.features.map((f) => (
                    <div key={f} style={{ fontSize: 15, color: W90, display: "flex", alignItems: "flex-start", gap: 8, lineHeight: 1.5 }}>
                      <span style={{ width: 5, height: 5, borderRadius: "50%", background: BLUE, flexShrink: 0, marginTop: 6, display: "block" }} />
                      {f}
                    </div>
                  ))}
                </div>
